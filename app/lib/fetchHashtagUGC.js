...
async function edgePage({hashtagId, edge="top_media", limit=10, after=""}){
  ...
  u.searchParams.set("user_id", IG_ID);
  // ⚠️ hashtag edges 不要 username，避免 (#100)
...
/* ==================================================================== */
/* ========== Hashtag：一次扫描直到命中全部 targetIds 即停 ============ */
/* ==================================================================== */
export async function scanHashtagUntil({
  tag,                  // string（不含 #）
  ...
  if (!IG_ID || !PAGE_TOKEN) return { hits: new Map(), scanned: 0, pages: 0, done: true };

  const hid = await getHashtagId(String(tag||"").replace(/^#/,""));
  if (!hid) return { hits: new Map(), scanned: 0, pages: 0, done: true };
  ...
  while (goals.size > 0 && scanned < maxScan && pages < hardPageCap) {
    const { items, nextAfter } = await edgePage({ hashtagId: hid, edge: "recent_media", limit: per, after });
...
export async function scanTagsUntil({
  ...
  while (goals.size > 0 && scanned < maxScan && pages < hardPageCap) {
    ...
    for (const m of data) {
      ...
      goals.delete(id);
      if (goals.size === 0) break; // ✅ 全部命中，提前停
    }

    if (goals.size === 0) break;
    after = j?.paging?.cursors?.after || "";
    if (!after) break; // 没下一页了
  }
