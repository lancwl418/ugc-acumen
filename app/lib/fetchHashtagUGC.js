...
export async function fetchTagUGCPage({ limit=12, after="" } = {}) {
  ...
  return withCache(key, 30_000, async () => {
    ...
    u.searchParams.set(
      "fields",
      "id,caption,media_type,media_url,thumbnail_url,permalink,timestamp,username,like_count,comments{id,text,username,timestamp},children{media_type,media_url,thumbnail_url}"
    );
